...
import Note from './NoteDraggable';
   ...
   */
	...
	componentWillMount() {
		...
		fetch(`https://baconipsum.com/api/?type=all-meat&sentences=${this.props.count}`)
			...
			.then((jsonResponse) =>
				jsonResponse[0].split('. ').forEach((sentence) => self.add(sentence.substring(0, 20)))
			);
	}
	...
	update(newText, i) {
		...
		this.setState((prevState) => ({
			board_notes: prevState.board_notes.map((note) => (note.id !== i ? note : { ...note, content: newText }))
		}));
	}
	...
	add(text) {
		this.setState((prevState) => ({
			board_notes: [
				...
				{
					content: text,
					// if we use the length of the array, then we end up getting the size right but new Notes can end up with duplicate keys and index
					id: this.nextId()
				}
			]
		}));
